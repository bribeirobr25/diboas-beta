...
import { getGlobalApplication } from '../../../src/application/DiBoaSApplication.js'
...
export class DiBoaSUIManager {
  ...
  async _initializeUserFeatures() {
    ...
    // Initialize user-specific UI features based on user journey phase
    ...
    console.log(`✅ User features initialized for phase ${userPhase}`)
  }

  async _enablePhaseFeatures(phase) {
    // Enable UI features based on user phase progression
    const body = document.body
    body.setAttribute('data-user-phase', phase.toString())
    
    // Phase-specific feature enabling
    switch (phase) {
      case 1: // Aqua phase - basic features
        body.classList.add('phase-aqua')
        break
      case 2: // Verde phase - additional assets
        body.classList.add('phase-verde')
        break
      case 3: // Mystic phase - advanced features
        body.classList.add('phase-mystic')
        break
      case 4: // Coral phase - full access
        body.classList.add('phase-coral')
        break
    }
  }

  async _setupUserEventListeners() {
    // Add user-specific event listeners that weren't added in global setup
    const userSpecificElements = document.querySelectorAll('[data-user-action]')
    userSpecificElements.forEach(element => {
      const action = element.getAttribute('data-user-action')
      if (action && !this._eventListeners.has(element)) {
        const listener = (e) => this._handleUserAction(action, e)
        element.addEventListener('click', listener)
        this._eventListeners.set(element, () => element.removeEventListener('click', listener))
      }
    })
  ...
  async _initializePersonalizedMascot() {
    if (!this._currentUser.currentMascot) return
    
    // Update mascot personalization based on user preferences
    const mascotElements = document.querySelectorAll(this._selectors.mascotContainer)
    mascotElements.forEach(element => {
      element.setAttribute('data-personality', 'friendly')
      element.setAttribute('data-user-level', this._currentUser.currentPhase.toString())
    })
  }

  async _setupAchievementSystem() {
    // Initialize achievement notification system
    const achievementContainer = document.querySelector('.achievement-notifications')
    if (achievementContainer && this._currentUser.achievements) {
      achievementContainer.setAttribute('data-achievements', this._currentUser.achievements.length.toString())
    }
  }

  async _handleUserAction(action, event) {
    try {
      await this.handleUserInteraction(action, {
        element: event.target,
        timestamp: Date.now()
      })
    } catch (error) {
      console.warn(`User action ${action} failed:`, error.message)
    }
  }
